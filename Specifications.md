...
### 8.6 UI/UXの改善
- **状態管理の強化**:
  - **モード切り替え**: カタログモード、フォルダモード、Photosモード間の切り替え時に、選択状態（`selectedCatalogFolder`など）を適切にクリアし、表示の競合を防止。
  - **アプリ切り替え時の状態維持**: アプリがバックグラウンドから復帰した際、ライブラリモード閲覧中であればカタログ表示に戻らず、ライブラリ表示を維持。
- **永続化**:
  - **ペインサイズ**: サイドバーとインスペクタの幅をアプリ終了後も保持。
  - **ライブラリリスト**: 追加したPhotosライブラリを`UserDefaults`に保存。
- **ライフサイクル**:
### 8.7 画面表示モードの拡張 (v2.37 - v2.39)
- **フルスクリーンプレビュー**:
  - **ショートカット**: `F` キー
  - **機能**: サイドバーなどのUI要素を非表示にし、画像のみを全画面表示。
  - **操作**: 矢印キーによる画像切り替え、`I`キーによるインスペクタ表示、`Esc`による解除。Advanced Copy等の誤操作防止のためショートカット無効化。
- **Sub View (マルチディスプレイ対応)**:
  - **ショートカット**: `Option + F`
  - **機能**: セカンダリディスプレイにフルスクリーンプレビューウィンドウを表示。
  - **条件**: ディスプレイが2台以上接続されている場合のみ有効。
  - **同期**: メインウィンドウでの画像切り替えに瞬時に追従。

### 8.8 その他の改善と修正
- **カタログ選択時の状態管理**: カタログ選択時にPhotos Libraryの選択状態が残る問題を修正し、フィルタリングの競合を解消。
- **インスペクタショートカット**: 操作性を向上させるため、複雑なコンビネーションから単一キー `I` に変更。
- **シャッタースピードソート**: 文字列比較から数値比較（1/100, 0.5"など）へのパーサー実装による正しいソート順の実現。
